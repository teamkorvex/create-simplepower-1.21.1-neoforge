...
- **SU-consuming kinetic block + goggle overlay** — `content/kinetics/CreateSimplePowerKineticBlock` +
  `CreateSimplePowerKineticBlockEntity`. An encased-shaft-style block that joins the kinetic
  ...
  Its model is an inset casing and `CreateSimplePowerShaftRenderer` spins a Create shaft that pokes
...
- **Kinetic generator + goggle overlay** — `content/kinetics/CreateSimplePowerGeneratorBlock` +
  `CreateSimplePowerGeneratorBlockEntity`. The counterpart source: it *adds* stress capacity
  ...
  network and nothing turns. It shares `CreateSimplePowerShaftRenderer` and adds custom lines to the
  Engineer's Goggles tooltip (`IHaveGoggleInformation#addToGoggleTooltip`).
- **Display Link source** — `content/display/CreateSimplePowerDisplaySource` + `AllDisplaySources`.
...
- **Sequenced assembly recipe** — `datagen/CreateSimplePowerSequencedAssemblyGen`. An item is
  processed through a Deployer then a Mechanical Press, looping twice, using a
  transitional "incomplete" item.
- **Ponder plugin** — `content/ponder/CreateSimplePowerPonderPlugin` +
  `CreateSimplePowerPonderScenes`. Wires up Create's in-game animated manual for the addon and
  is registered client-side in `CreateSimplePower#onClientSetup`. Ships a worked scene driven
  by the `assets/createsimplepower/ponder/createsimplepower_ponder.nbt` schematic (two Mechanical Arms and
  ...
  (`CreateSimplePower#registerPonderLang`), so it appears after `./gradlew runData`.

- **Create-style item tooltip** — the `createsimplepower_item`'s tooltip is authored as lang keys in
  `assets/createsimplepower/lang/default/tooltips.json` (`.tooltip.summary` +
  `.tooltip.condition1`/`.behaviour1`). The `ItemDescription` modifier set up on the
  Registrate in `CreateSimplePower` reads them and adds the "Hold Shift" prompt and highlight
...
- **Lang: keys in code, copy in JSON** — like Create, English strings live in hand-authored
  partials under `assets/createsimplepower/lang/default/` (`interface.json`, `tooltips.json`), while
  `Lang` holds only translation keys. `datagen/CreateSimplePowerLangMerger` merges the partials into the
  ...
  `CreateSimplePowerLangMerger`) — never by putting English in Java.
- **Additional languages** — `assets/createsimplepower/lang/de_de.json` is a worked German
...
It clones this template and rebrands every `Example` / `createsimplepower` reference to your
...
Run the bootstrap task once — it rewrites every `Example` / `createsimplepower` reference,
...
2. Rename `src/main/java/com/example/createsimplepower/` to match your `mod_group_id`
3. Update `CreateSimplePower.java` — change `ID` to your `mod_id`
4. Rename `src/main/resources/createsimplepower.mixins.json` to `{mod_id}.mixins.json` and
...
1. Go into your CreateSimplePower.java class to get a quick overview over what is defined and where exactly.
