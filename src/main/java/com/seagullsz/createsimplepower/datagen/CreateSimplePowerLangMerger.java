 ...
 * {@code assets/createsimplepower/lang/default/*.json}, and only the translation <em>keys</em>
 * appear in code (see {@link com.seagullsz.createsimplepower.Lang}).
 *
 ...
 * into that same provider via {@link CreateSimplePower#registerLangPartials()}, so the partials
 ...
 */
public class CreateSimplePowerLangMerger {

    private static final Gson GSON = new Gson();

    /** Partial file names under {@code assets/createsimplepower/lang/default/}, without ".json". */
    ...
    public static void mergeInto(BiConsumer<String, String> consumer) {
        for (String partial : PARTIALS) {
            String path = "assets/" + CreateSimplePower.ID + "/lang/default/" + partial + ".json";
            try (InputStream in = CreateSimplePowerLangMerger.class.getClassLoader().getResourceAsStream(path)) {
                if (in == null) {
                    CreateSimplePower.LOGGER.error("Lang partial not found on the classpath: {}", path);
                    continue;
                }
